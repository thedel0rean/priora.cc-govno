...
#include "..\..\visuals\esp\other_esp.h"
...
float aim::GetBodyScale(player_t* player)
{
	...
	(player->m_vecOrigin().DistTo(g_ctx.local()->get_eye_pos()), 0.f, g_ctx.globals.weapon->get_csweapon_info()->flRange / 4.f);
...
void aim::fire(CUserCmd* cmd)
{
	...
	log << crypt_str("hitchance: ") + (hitchance_amount == 101 ? crypt_str("MA") : std::to_string(hitchance_amount)) + crypt_str(", ");

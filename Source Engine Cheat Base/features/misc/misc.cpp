...
#include "misc.h"
#include "..\ragebot\anti-aim\fakelag.h"
#include "..\ragebot\aim\aim.h"
#include "..\visuals\world\world_esp.h"
#include "animation fix/prediction_system.h"
#include "logs.h"
#include "../ragebot/autowall/autowall2.h"
...
#endif
#else
#define VERSION crypt_str("legendware.pw | ")
#endif
...
void misc::ChatSpamer()
{
	...
	{ 
		crypt_str("legendware.pw - better than all"),
	};

	static auto lastspammed = 0;

	if (GetTickCount() - lastspammed > 930)
	{
...
{
	if (g_ctx.local()->m_vecVelocity().Length2D() < 0.25f)
	{
		g_ctx.m_bIsLocalPeek = false;
		// break lc
		g_ctx.globals.tickbase_shift = 4;
		return;
	}
	...
	for (auto i = 1; i <= m_globals()->m_maxclients; i++)
	{
		...
		for (int next_chock = 1; next_chock <= m_clientstate()->iChokedCommands; ++next_chock)
		{
			...
			fire_data.damage = CAutoWall::GetDamage(predicted_eye_pos, e, e->hitbox_position_matrix(HITBOX_HEAD, record->matrixes_data.first), &fire_data);
			...
			if (fire_data.damage < 1) continue;
	...
	if (g_ctx.m_bIsLocalPeek)
	{
		if (!g_ctx.globals.m_bInDiffensiveDt)
		{
			...
			g_ctx.globals.tickbase_shift = g_ctx.globals.weapon->get_max_tickbase_shift();
			return;
		}
	}
	else
...
bool misc::double_tap(CUserCmd* m_pcmd)
{
	...
	Vector m_vecAngles;
	m_engine()->GetViewAngles(m_vecAngles);
	...
	if (!g_ctx.globals.weapon->is_grenade() && g_ctx.globals.weapon->m_iItemDefinitionIndex() != WEAPON_TASER && g_ctx.globals.weapon->m_iItemDefinitionIndex() != WEAPON_REVOLVER && g_ctx.globals.weapon->m_iItemDefinitionIndex() != WEAPON_HEALTHSHOT && g_ctx.globals.weapon->m_iItemDefinitionIndex() != WEAPON_C4 && !g_ctx.globals.weapon->is_knife() && g_ctx.send_packet && m_pcmd->m_buttons & IN_ATTACK) //-V648
	{

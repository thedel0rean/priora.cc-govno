...
#include "other_esp.h"
#include "..\..\ragebot\autowall\autowall.h"
...
void otheresp::automatic_peek_indicator()
{
	...
	if (!weapon->is_non_aim() && key_binds::get().get_key_bind_state(18) || alpha)
	{
		...
		render::get().Draw3DCircle(position, 18.0f, g_ctx.globals.fired_shot ? Color(30, 240, 30, (int)(alpha * 255.0f)) : Color(240, 30, 30, (int)(alpha * 255.0f)));
	...
	if (!weapon->is_non_aim() && key_binds::get().get_key_bind_state(18) || alpha)
	{
		...
		for (int i = 1; i < max(2, 24 * alpha); i++) {
			if (rad > 2.f)
				g_Render->DrawRing3D(position.x, position.y, position.z, round(rad), 180, Color(0, 00, 0, 0), Color(g_cfg.misc.automatic_peek_color.r(), g_cfg.misc.automatic_peek_color.g(), g_cfg.misc.automatic_peek_color.b(), int(i * alpha)), 0, 0.f);

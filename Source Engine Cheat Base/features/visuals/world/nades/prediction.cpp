...
inline float g_ctx_Armor(float flDamage, int ArmorValue) {
...
void GrenadePrediction::Paint()
{
	...
	Vector prev = path[0];
	Color trace_color = g_cfg.esp.grenade_prediction_color;
	for (int i{ 1 }; i < m_globals()->m_maxclients; ++i) {
		...
		if (weapon->m_iItemDefinitionIndex() == WEAPON_HEGRENADE) {
			...
			if (ptr.hit_entity == player) {
				...
				auto dmg = max(static_cast<int>(ceilf(g_ctx_Armor(flDamage, player->m_ArmorValue()))), 0);
	...
	if (he_target.second && weapon->m_iItemDefinitionIndex() == WEAPON_HEGRENADE && (!g_cfg.esp.on_click || g_ctx.get_command()->m_buttons & IN_ATTACK || g_ctx.get_command()->m_buttons & IN_ATTACK2)) {
		Vector screen;
		...
		//57
		g_Render->DrawString(w / 2, h / 2 - 75, (int)he_target.first >= he_target.second->m_iHealth() ? Color::Red : Color::White, render2::centered_x | render2::centered_y | render2::outline, c_menu::get().font, text.c_str());
			//g_Render->DrawString(screen.x, screen.y, (int)he_target.first >= he_target.second->m_iHealth() ? Color::Red : Color::White, HFONT_CENTERED_X | HFONT_CENTERED_Y, c_menu::get().font, text.c_str());
	}
	else if (molo_target.second && weapon->m_iItemDefinitionIndex() == (WEAPON_MOLOTOV || WEAPON_INCGRENADE) && (!g_cfg.esp.on_click || g_ctx.get_command()->m_buttons & IN_ATTACK || g_ctx.get_command()->m_buttons & IN_ATTACK2)) {
		Vector screen;

...
void worldesp::skybox_changer()
{
	...
	case 14:
		...
		break;
	case 15:
		skybox_name = "sky_g_ctx_cloudy01";
		break;
	case 16:
		skybox_name = "sky_g_ctx_night02";
		break;
	case 17:
		skybox_name = "sky_g_ctx_night02b";
		break;
	case 18:
		skybox_name = "sky_g_ctx_night_flat";
		break;
	case 19:
...
void worldesp::bomb_timer(entity_t* entity)
{
	...
	if (!g_ctx.globals.bomb_timer_enable)
		g_ctx.globals.C4 = crypt_str("unPlanted");
	if (g_ctx.globals.C4 == crypt_str("unPlanted"))
		return;
}
...
void worldesp::dropped_weapons(entity_t* entity)
{
	...
	if (util::get_bbox(weapon, box, false))
	{
		...
		if (g_cfg.esp.weapon[WEAPON_BOX])
		{
			...
			{
				g_Render->DrawString(box.x + box.w / 2, box.y - 14, g_cfg.esp.weapon_color, HFONT_CENTERED_X | render2::outline, c_menu::get().weapon_icons, weapon->get_icon());
				offset = 14;
			}

			if (g_cfg.esp.weapon[WEAPON_TEXT])
				g_Render->DrawString(box.x + box.w / 2, box.y + box.h + 2, g_cfg.esp.weapon_color, HFONT_CENTERED_X | render2::outline, c_menu::get().esp_name, weapon->get_name().c_str());
			...
			if (g_cfg.esp.weapon[WEAPON_AMMO] && entity->GetClientClass()->m_ClassID != CBaseCSGrenadeProjectile && entity->GetClientClass()->m_ClassID != CSmokeGrenadeProjectile && entity->GetClientClass()->m_ClassID != CSensorGrenadeProjectile && entity->GetClientClass()->m_ClassID != CMolotovProjectile && entity->GetClientClass()->m_ClassID != CDecoyProjectile)
			{
				...
				g_Render->FilledRect(box.x, box.y + box.h + 15, weapon->m_iClip1() * box.w / weapon->get_csweapon_info()->iMaxClip1, 2, g_cfg.esp.weapon_ammo_color);
			}

			if (g_cfg.esp.weapon[WEAPON_DISTANCE])
			{
				auto distance = g_ctx.local()->GetAbsOrigin().DistTo(weapon->GetAbsOrigin()) / 12.0f;
				g_Render->DrawString(box.x + box.w / 2, box.y - 13 - offset, g_cfg.esp.weapon_color, HFONT_CENTERED_X | render2::outline, c_menu::get().font, "%i m", (int)distance / 3.14);
			}
		}
		else
		{
			if (g_cfg.esp.weapon[WEAPON_ICON])
				g_Render->DrawString(box.x + box.w / 2, box.y + box.h / 2 - 7, g_cfg.esp.weapon_color, HFONT_CENTERED_X | render2::outline, c_menu::get().weapon_icons, weapon->get_icon());

			if (g_cfg.esp.weapon[WEAPON_TEXT])
				g_Render->DrawString(box.x + box.w / 2, box.y + box.h / 2 + 6, g_cfg.esp.weapon_color, HFONT_CENTERED_X | render2::outline, c_menu::get().esp_name, weapon->get_name().c_str());
			...
			if (g_cfg.esp.weapon[WEAPON_DISTANCE])
			{
				...
				g_Render->DrawString(box.x + box.w / 2, box.y + box.h / 2 - offset, g_cfg.esp.weapon_color, HFONT_CENTERED_X | render2::outline, c_menu::get().font, "%i m", (int)distance / 3.14);
			}
		}
	}
}
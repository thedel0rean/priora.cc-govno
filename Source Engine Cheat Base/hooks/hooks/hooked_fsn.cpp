...
#include "..\..\features\ragebot\aim\aim.h"
...
#include "..\..\features\ragebot\anti-aim\fakelag.h"
#include "..\..\features\visuals\world\world_esp.h"
#include "..\..\features\misc\logs.h"
#include "..\..\features\misc\animation fix\prediction_system.h"
...
void __stdcall hooks::hooked_fsn(ClientFrameStage_t stage)
{
	...
	if (stage == FRAME_NET_UPDATE_END)
	{
		...
		if (current_shot != g_ctx.shots.end())
		{
			if (!current_shot->latency)
			{
				...
				if (!current_shot->hurt_player)
				{
					...
					else if (g_cfg.misc.events_to_log[EVENTLOG_HIT])
					{
						...
						else if (current_shot->shot_info.backtrack_ticks >= 15)
							eventlogs::get().add(crypt_str("Missed shot due to backtrack failure"));
						else if (current_shot->shot_info.backtrack_ticks <= 14)
							eventlogs::get().add(crypt_str("Missed shot due to lagcompensation failure"));
						else
			...
			{
				eventlogs::get().add(crypt_str("Missed shot due to unregistred shot"));
			}

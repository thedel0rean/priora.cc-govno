...
bool ImGui::tab(const char* icon, const char* label, bool selected)
{
    ...
    const float tab_height = 25.0f; //  высота вкладки

    const ImRect rect(pos, ImVec2(pos.x + label_size.x + 5, pos.y + tab_height));
    ...
    // –исуем текст поверх квадратика
    ImGui::PushFont(c_menu::get().cray);
    window->DrawList->AddText(ImVec2(rect.Min.x + 10, rect.GetCenter().y - label_size.y / 2), ImColor(141, 149, 255, 255), icon);
    ImGui::PopFont();
    ImGui::PushFont(c_menu::get().fontx);
    window->DrawList->AddText(ImVec2(rect.Min.x + 35, rect.GetCenter().y - label_size.y / 2), ImColor(141, 149, 255, 255), label);
    ImGui::PopFont();
    ...
    GetWindowDrawList()->AddRectFilled(rect.Min, rect.Max, GetColorU32(hover_dis2), 4.f);

    /*
    window->DrawList->AddText(ImVec2((rect.Min.x + rect.Max.x) / 2.f - (label_size.x / 2.f), (rect.Min.y + rect.Max.y) / 2.f - (label_size.y / 2.f)),
        selected ? ImColor(141, 149, 255, (160 + it_anim->second.hovered_text_anim + it_anim->second.active_text_anim)) : ImColor(255, 255, 255, (160 + it_anim->second.hovered_text_anim + it_anim->second.active_text_anim)),
        label);
...
bool ImGui::TabEx(const char* icon, const char* label, int number, bool* active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    // ImGui::Spacing(5);
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    if (*active) {
        x = 0;
        hover_dis2 = ImColor(217, 217, 217, 12);
        text1 = ImColor(141, 149, 255, 255);
    }
    else {
        x = 0;
        text1 = ImColor(255, 255, 255, 260);
        hover_dis2 = ImColor(7, 7, 7, 200);
    }
    ...
    ImGui::PushFont(c_menu::get().fontx);
    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 49 * c_menu::get().dpi_scale + x, bb.Min.y + 9 * c_menu::get().dpi_scale), GetColorU32(text1), label);
    ImGui::PopFont();

    ImGui::PushFont(c_menu::get().cray);
    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 12 * c_menu::get().dpi_scale + x, bb.Min.y + 9 * c_menu::get().dpi_scale), GetColorU32(text1), icon);
    ImGui::PopFont();
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, window->DC.LastItemStatusFlags);
    return pressed;
}

bool ImGui::Tab(const char* icon, const char* label, int number, bool active, const ImVec2& size_arg)
{
    return TabEx(icon, label, number, &active, size_arg, 0);
}
